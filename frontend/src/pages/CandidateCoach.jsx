    ...
    <div className="space-y-6 p-6">
      ...
      {mode === 'history' ? (
        <section className="glass-panel rounded-2xl p-5">
          <div className="mb-4 flex items-center justify-between"><h3 className="font-semibold text-white">Recent practice</h3><span className="text-xs text-slate-500">{history.length} practice sessions</span></div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,0.65fr)]">
          <section className="glass-panel rounded-2xl p-5">
            ...
            <div className="mt-5 flex flex-wrap gap-3"><button onClick={submit} disabled={loading} className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-60">{loading ? 'Reviewing your answer...' : 'Get feedback'}</button>{mode === 'simulation' && <button onClick={nextSimulationQuestion} className="rounded-lg border border-slate-700 px-4 py-2.5 text-sm text-slate-300 hover:border-slate-500">Next question</button>}</div>
          </section>
