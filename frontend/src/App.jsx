...
const MainLayout = ({ children }) => {
  ...
  // Unified Three-Role Sidebar Layout
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-slate-950 text-slate-100 relative">
      ...
      <header className="md:hidden flex items-center justify-between p-4 border-b border-slate-800/80 bg-slate-950/80 backdrop-blur-md sticky top-0 w-full z-40">
        ...
        <button
          ...
          className="p-1.5 border border-slate-855 hover:border-slate-800 bg-slate-900/60 text-slate-400 hover:text-slate-200 rounded-lg transition"
        >
      ...
      <main className="flex-1 overflow-y-auto max-h-screen relative w-full">
        ...
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-indigo-650/5 rounded-full blur-[120px] pointer-events-none z-0"></div>
...
const AuthLayout = ({ children }) => {
  ...
  if (user) {
    if (user.Role === 'Candidate') {
      ...
      return <Navigate to="/admin/assessments" replace />;
    }
...
const ProtectedRoute = ({ children, allowedRoles }) => {
  ...
  // Authorization check and redirect to respective homes on failure
